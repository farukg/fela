...
The ReScript bindings ship with some core Fela APIs, React hooks and all plugins, enhancers and presets.
...
```bash
yarn add rescript-fela

# you'll also need at least fela and react-fela
yarn add fela react-fela
```

In your `bsconfig.json`, include `"rescript-fela"` in the `bs-dependencies`.

## Creating a Fela renderer

First let's create our Fela [Renderer](basics/renderer) instance. We can pass all the [config](advanced/renderer-configuration) options that the Fela renderer accepts.

```rescript name=FelaRenderer.res
open Fela
...
let renderer = Renderer.make(
  RendererConfig.make(~plugins=Presets.web, ~selectorPrefix="rescript-", ()),
)
```
...
Now that our app is aware of the renderer, we can use the provided hooks in any component. The hooks are similar to react-fela's [useFela](api/react-fela/useFela), but split into 3 different hooks for convenience.

```rescript name=RedText.res
open ReactFela
...
Therefore, rescript-fela also provides bindings to fela-dom.

```rescript name=Server.res
open Fela.Dom
...
let make = (~children) => {
  ...
  renderer["renderStatic"](
    fromBsCssCore(style(list{backgroundColor(black)})),
    "body",
  )
  <div
    className={css(list{
      fromBsCssCore(
        style(list{fontSize(pt(18)), color(theme["colors"]["primary"])}),
      ),
    })}>
    {"I'm red"->React.string}
  </div>
...
- [reason-fela](https://github.com/robinweser/fela/tree/master/packages/reason-fela)
- [bs-css-fela](https://github.com/astrada/bs-react-fela)
